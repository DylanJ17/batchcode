...
def format_datetime(dt: datetime, include_day_name: bool = True) -> str:
    """Format datetime based on preference in session state ('UK' or 'US')."""
    pref = st.session_state.get('date_format', 'UK')
    fmt = '%m/%d/%Y' if pref == 'US' else '%d/%m/%Y'
    return dt.strftime(f"{fmt} (%A)") if include_day_name else dt.strftime(fmt)
...
with st.sidebar:
    ...
    # Date format toggle with callback
