...
# Main app layout
st.markdown("<div class='header-title'>📦 Philip Kingsley Batch Code Reader</div>", unsafe_allow_html=True)
st.markdown("<div style='text-align: center; color: #666; font-size: 1.1rem; margin-bottom: 30px;'>✨ Premium batch code analysis with intelligent pattern matching ✨</div>", unsafe_allow_html=True)
...
tab1, tab2, tab3, tab4 = st.tabs(["🔍 Single Code", "📊 Batch Process", "⚙️ Utilities", "📚 Help & Info"])
...
# Tab 3: Utilities
with tab3:
    st.markdown("### 🔧 Utilities & Tools")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        if st.button("🧪 Test Pattern Recognition", use_container_width=True, type="secondary"):
            st.subheader("🧪 Pattern Validation Results")
            ...
            for item in test_codes:
                ...
                results_test.append({
                    ...
                    'Status': status,
                    'Details': msg[:50] + '...' if len(msg) > 50 else msg
                })
            
            test_df = pd.DataFrame(results_test)
            st.dataframe(test_df, use_container_width=True, hide_index=True)
            
            passed = len([r for r in results_test if '✅' in r['Status']])
            st.markdown(f"<div class='success-box'>✅ Pattern tests: {passed}/{len(test_codes)} passed</div>", unsafe_allow_html=True)
    
    with col2:
        if st.button("📊 View Analysis Info", use_container_width=True, type="secondary"):
            st.subheader("📊 Analysis Information")
            
            info_data = {
                'Production Date Range': '2015 - Present + 1 day',
                'Expiry Calculation': 'Production Date + 3 years',
                'Total Patterns Supported': str(len(st.session_state.validator.historical_patterns)),
                'Date Formats Supported': 'DD/MM/YY, Julian Days, Mixed Alpha',
                'Confidence Scoring': 'Algorithmic pattern matching',
            }
    ...
    with col3:
        if st.button("💡 Quick Tips", use_container_width=True, type="secondary"):
            st.subheader("💡 Quick Tips & Tricks")
            
            tips = [
                "🔍 Use the test utility to verify pattern recognition",
                "📋 Batch processing can handle 100+ codes at once",
                "💾 Export results as CSV for spreadsheet analysis",
                "🎯 Highest confidence result is usually most accurate",
                "⏰ UK date format (DD/MM/YY) is used throughout",
                "🏭 All calculations assume 3-year shelf life",
            ]
...
with tab4:
    ...
    with col1:
        ...
        This premium batch code analyser recognises multiple Philip Kingsley batch code formats 
        and instantly determines production and expiry dates with high accuracy.
        
        #### Key Features
        
        ✨ **Real-Time Analysis** - Get instant feedback on individual codes  
        📊 **Batch Processing** - Analyse up to 1,000+ codes at once  
        🎯 **Pattern Matching** - Supports 7+ batch code formats  
        📈 **Confidence Scoring** - See how confident each result is  
        💾 **Export Functionality** - Download results as CSV  
        🌍 **Date Format Toggle** - UK (DD/MM/YY) throughout  
    ...
    with col2:
        ...
        ### 📊 Quick Stats
        """)
        st.metric("Patterns Supported", len(st.session_state.validator.historical_patterns))
        st.metric("Batch Capacity", "1,000+ codes")
        st.metric("Analysis Speed", "<100ms per code")
    
    st.divider()
    
    st.markdown("### 📌 All Supported Batch Code Formats")
    
    for name, info in st.session_state.validator.historical_patterns.items():
        with st.expander(f"📍 {name.replace('_', ' ').upper()}", expanded=False):
            st.markdown(f"**Description:** {info['description']}")
            
            examples_str = ", ".join([f"`{ex}`" for ex in info['examples']])
            st.markdown(f"**Example Codes:** {examples_str}")
    ...
    | ⚠️ Expired | Red | Product has expired |
    
    ### 🔢 Date Validation Rules
    
    - **Production Date Range:** 2015 to current year + 1 day
    - **Expiry Date Calculation:** Production date + 3 years
    - **Expiry Range:** Must fall within 2015 to current year + 3 years
    - **Future Production:** Maximum 90 days in advance
    
    ### 💾 Exporting Results
    
    When you batch process codes, you can download results as CSV for:
    - Integration with inventory systems
    - Historical tracking and reporting
    - Data analysis in Excel or other tools
