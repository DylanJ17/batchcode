...
if 'single_code_interpretations' not in st.session_state:
    st.session_state.single_code_interpretations = None


def format_datetime(dt: datetime, include_day_name: bool = True) -> str:
    """Format datetime based on user preference (UK/US) stored in st.session_state['date_format']. Defaults to UK."""
    pref = st.session_state.get('date_format', 'UK')
    fmt = '%m/%d/%Y' if pref == 'US' else '%d/%m/%Y'
    if include_day_name:
...
# Sidebar
with st.sidebar:
    # Show only the date display format toggle
    def update_date_format():
        selected = st.session_state.date_format_radio
        st.session_state['date_format'] = selected.split()[0]

    st.radio(
        '📅 Date Display Format',
        options=['UK (DD/MM/YY)', 'US (MM/DD/YY)'],
        index=0 if st.session_state.get('date_format','UK') == 'UK' else 1,
        key='date_format_radio',
        on_change=update_date_format
    )
...
with tab1:
    ...
    if analyse_btn or code_input:
        if code_input:
            ...
            if valid and interpretations:
                ...
                # Store interpretations for dynamic rendering
                st.session_state.single_code_interpretations = interpretations
                
                # Store results with raw datetime objects for dynamic formatting
                st.session_state.results_df = pd.DataFrame([{
                    'Code': code_input,
                    'Format': interpretations[0].get('format_type', 'N/A'),
                    'Production Date (Raw)': interpretations[0]['production_date'],
                    'Expiry Date (Raw)': interpretations[0]['expiry_date'],
                    'Days To Expiry': (interpretations[0]['expiry_date'].date() - datetime.now().date()).days,
                    'Expiry Status': get_expiry_status((interpretations[0]['expiry_date'].date() - datetime.now().date()).days)[0],
                    'Confidence': f"{interpretations[0].get('confidence', 0):.0%}",
                    'Details': f"Format: {interpretations[0].get('format_type', 'N/A')}"
                }])
            else:
    ...
    # Display stored interpretations (allows dynamic date format updates)
    if st.session_state.single_code_interpretations:
        for i, result in enumerate(st.session_state.single_code_interpretations, 1):
            with st.container(border=True):
                ...
                with col1:
                    prod_dt = result['production_date']
                    st.markdown(f"🏭 **Production Date**\n\n{format_datetime(prod_dt)}")
                with col2:
                    exp_dt = result['expiry_date']
                    st.markdown(f"📅 **Expiry Date**\n\n{format_datetime(exp_dt)}")
        ...
        # Show download button for single code
        st.divider()
        col1, col2, col3 = st.columns([1, 1, 2])
        with col1:
            # Format dates for CSV export
            export_df = st.session_state.results_df.copy()
            if 'Production Date (Raw)' in export_df.columns:
                export_df['Production Date'] = export_df['Production Date (Raw)'].apply(lambda x: format_datetime(x, include_day_name=False) if pd.notna(x) else 'N/A')
                export_df = export_df.drop('Production Date (Raw)', axis=1)
            if 'Expiry Date (Raw)' in export_df.columns:
                export_df['Expiry Date'] = export_df['Expiry Date (Raw)'].apply(lambda x: format_datetime(x, include_day_name=False) if pd.notna(x) else 'N/A')
                export_df = export_df.drop('Expiry Date (Raw)', axis=1)
...
with tab2:
    ...
    if process_btn:
        ...
        if not codes:
            ...
            for idx, code in enumerate(codes):
                ...
                if valid and interpretations:
                    ...
                    results_list.append({
                        ...
                        'Production Date (Raw)': top['production_date'],
                        'Expiry Date (Raw)': top['expiry_date'],
            ...
            # Format dates dynamically based on current setting
            if 'Production Date (Raw)' in display_df.columns:
                display_df['Production Date'] = display_df['Production Date (Raw)'].apply(lambda x: format_datetime(x, include_day_name=False) if pd.notna(x) else 'N/A')
                display_df = display_df.drop('Production Date (Raw)', axis=1)
            
            if 'Expiry Date (Raw)' in display_df.columns:
                display_df['Expiry Date'] = display_df['Expiry Date (Raw)'].apply(lambda x: format_datetime(x, include_day_name=False) if pd.notna(x) else 'N/A')
                display_df = display_df.drop('Expiry Date (Raw)', axis=1)
